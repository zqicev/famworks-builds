...
```json
{
  ...
  "mods": [ ... ],
  "servers": [
    { "name": "FamWorks", "ip": "play.example.com", "port": 25565 }
  ],
  "configs": [
...
`fabric_api_version` — лаунчер сам скачает Fabric API именно этой версии (добавлять в `mods` не нужно).

### Серверы (`servers`)

Добавляются в список мультиплеера игрока (`servers.dat`) при запуске. Серверы,
которые игрок добавил сам, сохраняются.
...
| `download_url` | ссылка на файл (в Releases famworks-builds) |
